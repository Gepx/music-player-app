...
class SpotifyApiTestController extends GetxController {
  ...
  final String testAlbumId = '4aawyAB9vmqN3uQ7FjRGTy'; // Hot Fuss by The Killers
    ...
    try {
      ...
      addResult(TestResult(
        ...
        data: {
          'total': result.total,
          'limit': result.limit,
          'tracks': result.items.take(5).map((t) => {
                'name': t.name,
                'artists': t.artists.map((a) => a.name).join(', '),
              }).toList(),
        },
  ...
  Future<void> testSaveTrack() async {
    ...
    try {
      ...
      addResult(TestResult(
        ...
        message: success ? 'Track saved successfully' : 'Failed to save track',
        timestamp: _formatTimestamp(),
      ));
    } catch (e) {
      addResult(TestResult(
  ...
  Future<void> testCache() async {
    ...
    try {
      ...
      addResult(TestResult(
        ...
        data: {
          'statistics': stats,
          'totalSize': _cache.getTotalCacheSize(),
        },
        timestamp: _formatTimestamp(),
      ));
    } catch (e) {
